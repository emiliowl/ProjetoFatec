    ...
    <body>
        ...
        <div id="conteudo" align="center">
            ...
            <?php
                ...
                if($coluna=="id_materia") {
                    $materiaDAO = new MateriaDAO();
                    $materia = $materiaDAO->buscaPorColuna($item, "nome");
                    $item = $materia->getMateriaID();
                } elseif ($coluna=="data") {
                    $novaData = substr($item, 0, 2);
                    $novaData = substr($item, 3, 2)."/".$novaData;
                    $novaData = substr($item, 6, 4)."/".$novaData;
                    $item = $novaData;
                }
                $aulaDAO = new AulaDAO();
                $aulas = $aulaDAO->buscarPorColuna($item, $coluna);
                if(isset($aulas)) {
            ?>
            <h1>Aulas Localizadas</h1>
            <br />
            <table  id="tbCadastros" width="100%" align="left">
                ...
                <tr align="center" style="font-size:10px;" <?php if($i % 2 == 0) { ?> bgcolor="lightgray"<?php } ?>>
                    ...
                    <td> <?php echo $aulas[$i]->getHoraFim(); ?> </td>
